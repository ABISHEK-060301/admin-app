...
module.exports = {
    ...
    createNewUser: (data, userId) => {
        const date = new Date();
        const { first_name, last_name, email, imageurl, role, address, country, state, city, } = data;
        return new Promise((resolve, reject) => {
            const sql = `INSERT INTO users (id, userid, first_name, last_name, email, imageurl, role, address, country, state, city, created_at, updated_at)
            VALUES (uuid(),?,?,?,?,?,?,?,?,?,?,?,?)`;
            pool.query(sql, [userId, first_name, last_name, email, imageurl, role, address, country, state, city, date, date], (error, result) => {
                if (error) {
                    return reject(error);
                }
                return resolve(result);
